...
`v2.1.0` · Windows · Electron · React · TypeScript · SQLite · DeepSeek
...
- 收藏单个英文单词，规范大小写并将常见复数归一为单数；重复词不会重复入库。
- 编辑英式 IPA、多个词性与中文释义、主分类和标签；字段有效时会自动保存。
- 搜索单词、IPA、释义、分类、标签、词素和构词说明，并按最近更新、A–Z 或记忆曲线排序。
- 在独立 Review Mode 中先回忆英文单词，再显示答案，用 `1`–`4` 选择“忘记、困难、记得、简单”；普通点击词卡只查看和编辑，不会记录复习。
...
2. 下载 `shengciben-setup-2.1.0.exe`。
3. 运行安装程序并选择安装目录。
...
安装包生成在 `release/` 目录，文件名为 `shengciben-setup-2.1.0.exe`。

## 使用

1. 点击“添加单词”，或按 `Ctrl+N`，输入一个英文单词并加入队列。
2. 在中栏搜索、筛选或排序；按 `Ctrl+K` 可直接聚焦搜索框。点击单词只会打开详情，不会改变复习状态。
3. 在右栏编辑 IPA、义项、分类和标签；字段有效时会自动保存。
4. 点击左侧“开始复习”，进入独立复习页面：先看英文单词，按 `Space` 或点击“显示答案”，再用 `1`–`4` 评分。
5. 复习中按 `Esc` 可退出；已评分的单词会立即保存，剩余单词下次重新从数据库构建队列。

Review Mode 的队列包含全部到期词，以及每日上限允许加入的新词。到期词不受每日新词上限限制；没有可复习单词时显示“今日完成”。
...
tests/         数据库、复习、归一化、AI、队列、导出与词根索引测试
